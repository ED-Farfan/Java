...
class MultiplicacionM {
  ...
  static long checkC = 0;
  static int FI = 0;
  static long[][] MatA = null;
  static long[][] MatB = null;
  static long[][] MatC = null;
  ...
  static class Worker extends Thread {
    ...
    public void run() {
      try {
        ...
        byte[] aaa = b.array();
        salida.write(aaa);
        //Creamos el espacio para mandar 2 Matrices que es 8 por que un long utiliza 8 bits, (2(8*(Filas*Columnas)))
        ByteBuffer bb = ByteBuffer.allocate(2 * (8 * N * FI));
        ...
        byte[] m = bb.array();
        salida.write(m);
        //Creamos el espacio para recibir una parte de la matriz C. Un entero utiliza 8 bits, ((8*(Filas*Fila)))
        byte[] mc = new byte[8 * (FI * FI)];
        read(entrada, mc, 0, 8 * (FI * FI));
        ByteBuffer bbb = ByteBuffer.wrap(mc);
        for (int i = Mnodos[nodo - 1][1]; i < Mnodos[nodo - 1][2]; i++) {
          for (int j = Mnodos[nodo - 1][3]; j < Mnodos[nodo - 1][4]; j++) {
            MatC[i][j] = bbb.getLong();            
          }          
        }
        //Creamos el espacio para recibir la suma de una parte de la matriz C
        byte[] rsc = new byte[8];
        read(entrada, rsc, 0, 8);
        ByteBuffer rsmc = ByteBuffer.wrap(rsc);
        long psumac = rsmc.getLong();
        synchronized (lock) {
          checkC += psumac;
        }
        Date date2 = new Date();
        System.out.println(
          ...
          Mnodos[nodo - 1][3]+" -> "+Mnodos[nodo - 1][4] +"] En SumaC:"+psumac+ 
          "\n"
        );        
        //
        salida.close();
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (args.length == 2 && nodo == 0) { //Es el servidor
      ...
      MatA = new long[Filas][Columnas];
      MatB = new long[Filas][Columnas];
      MatC = new long[Filas][Filas];
      System.out.println("Filas: " + Filas + " Columnas: " + Columnas);
      // Un long es de 8 bits, por lo que tenemos que reservar el espacio para 2 matrices de nFilas*nColumnas
      byte[] aa = new byte[8 * (Filas * Columnas) * 2];
      read(entrada, aa, 0, 8 * 2 * (Filas * Columnas));
      ...
      //Creamos el espacio para mandar la matriz C, 8*Filas*Filas
      ByteBuffer mc = ByteBuffer.allocate(8 * (Filas * Filas));
      for (int i = 0; i < Filas; i++) {
      ...
      ByteBuffer rsc = ByteBuffer.allocate(8);
      rsc.putLong(sumC);
      byte[] sc = rsc.array();
      salida.write(sc);
      salida.close();
  ...
  static void Mostrar(int F, int C, long[][] Matriz) {    
    if(C< 16){
      System.out.print("\n");
      for (int j = 0; j < F; j++) {
        for (int i = 0; i < C; i++) {
  ...
  static long Multiplicar(int F, int C,long [][]A,long [][]B,long [][]MC) {    
    long sumC=0;
    for (int i = 0; i < F; i++) 
        for (int j = 0; j < F; j++) 
            {for (int k = 0;k < C;k++ ){ 
  ...
  static long Sumar(int F, int C,long [][]MC ) {    
    int sumC=0;
    for (int i = 0; i < F; i++) 
        for (int j = 0; j < F; j++)             
            sumC+=MC[i][j];
